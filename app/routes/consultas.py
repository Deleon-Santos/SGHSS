...
def finaliza_consulta(consulta_id):
	...
	consultas_db.session.commit()
	return jsonify({"mensagem": f"Consulta {consulta_id} finalizada e diagnóstico registrado."}), 200




@consultas_db.route('/consulta/<int:consulta_id>/prescreve', methods=['POST'])
def prescreve_medicamento(consulta_id):
    ...
    consultas_db.session.add(medicamento)
    consultas_db.session.commit()
...
@consultas_db.route('/consulta/<int:consulta_id>/solicita_exame', methods=['POST'])
def solicita_exame(consulta_id):
    ...
    consultas_db.session.add(exame)
    consultas_db.session.commit()
...
@consultas_db.route('/agenda/medico/<int:medico_id>', methods=['GET'])
def consulta_agenda(medico_id):
    """
    Consulta a agenda do médico.
